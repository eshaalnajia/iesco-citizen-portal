...
export function MobileBottomSheet({
  ...
  return (
    <>
      ...
      <div
        ref={sheetRef}
        className="absolute bottom-0 left-0 right-0 z-40 bg-white dark:bg-card rounded-t-2xl shadow-2xl animate-in slide-in-from-bottom duration-300"
      ...
      >
        ...
        <div className="px-5 pb-3 border-b border-slate-100 flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="font-bold text-slate-900 dark:text-foreground">{feeder.name}</h2>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 text-slate-400 dark:text-muted-foreground hover:text-slate-600 dark:text-muted-foreground hover:bg-slate-100 rounded-lg flex-shrink-0"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
